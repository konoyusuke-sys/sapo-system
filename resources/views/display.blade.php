...
<head>
  ...
  <script>
    function init() {
      ...
      let mytext = localStorage.getItem("person-info");
      mytext = JSON.parse(mytext);
      var cur = 0;
      let inputArry = document.getElementsByTagName("input");
      for (let i = 0; i < inputArry.length; i++, cur++) {
        
        inputArry[i].setAttribute("value", mytext[i].value);
      }
      let selectArray = document.getElementsByTagName("select");
      for (let i = 0; i < selectArray.length; i++, cur++) {
        let cnt = selectArray[i].options.length;
        for (let j = 0; j < cnt; j++) {
          if (
            selectArray[i].getElementsByTagName("option")[j].value ==
            mytext[18 + i].value
          ) {
            selectArray[i].getElementsByTagName("option")[j].selected = true;
            break;
          }
        }
      }
      let textArray = document.getElementsByTagName("textarea");
      for (let i = 0; i < textArray.length; i++, cur++) {
        textArray[i].innerHTML = mytext[cur + 1].value;
      }
      return true;
    }
    
    function tosubmit(){
        localStorage.setItem('formSubmitted', 'thanks');
...
<body onload="init();">
  ...
  <main>
    <section id="application" class="application-sec" >
      ...
      <div class="">
        <form name="lifeform" method="post" onsubmit="tosubmit();" action="{{route('post_form')}}" enctype="multipart/form-data" class="area_form h-adr"
          id="form" novalidate>
          @csrf
          <dl class="req">
            <dt>お名前</dt>
            <dd class=" col2">
              <span>姓
                <input readonly class="chk w50 validate[required]" type="text" id="your_name_sei" name="name_sei"
                  value="" placeholder="例）○○" /></span>
              <span>名
                <input readonly class="chk w50 validate[required]" type="text" id="your_name_mei" name="name_mei"
                  value="" placeholder="例）●●" /></span>
            </dd>
          </dl>
          ...
          <dl class="req">
            <dt>お名前（ふりがな）</dt>
            <dd>
              <input readonly class="chk validate[required]" type="text" id="your_name_kana" name="name_kana" value=""
                placeholder="例）○○ ●●" />
            </dd>
          </dl>
          ...
          <dl class="req">
            <dt>メールアドレス</dt>
            <dd>
              <input readonly type="email" class="chk validate[required,custom[email]] m_b5" id="your_mail"
          ...
          <dl class="any">
            <dt>LINE ID</dt>
            <dd>
              <input readonly type="text" class="" id="your_line" name="your_line" value="" placeholder="例）abcd1234" />
              <p class="caption">
                半角英数字で入力してください<br />
          ...
          <dl class="req">
            <dt>郵便番号</dt>
            <dd>
              <input readonly class="p-postal-code validate[required] chk" type="text" id="your_postnum" name="your_postnum"
                value="" />
            </dd>
          </dl>
          <dl class="req">
            ...
            <dl class="req">
                <dt>建物名・部屋番号</dt>
                <dd>
                    <input class="chk " type="text" id="addr3" name="addr3" value=""
                        placeholder="0-00" />
                    <p class="caption">
          ...
          <dl class="req">
            <dt>携帯電話番号</dt>
            <dd>
              <input readonly class="validate[required] chk" type="text" id="your_mobile" name="your_mobile" value=""
          ...
          <dl class="any">
            <dt>固定電話番号</dt>
            <dd>
              <input readonly class="" type="text" id="your_tel" name="your_tel" value="" placeholder="例）0312345678" />
          ...
          <dl class="req">
            <dt>勤務先名</dt>
            <dd>
              <input readonly class="chk validate[required]" type="text" id="your_company" name="your_company" value=""
          ...
          <dl class="req">
            <dt>勤務先電話番号</dt>
            <dd>
              <input readonly class="validate[required] chk" type="text" id="your_company_tel" name="your_company_tel"
                value="" placeholder="例）0312345678" />
              <p class="caption">半角英数字で入力してください</p>
            </dd>
          </dl>
          ...
          <dl class="req">
            <dt>他者借入件数</dt>
            <dd>
              <input readonly class="validate[required] chk" type="text" id="your_borrowing_num" name="your_borrowing_num"
                value="" placeholder="例）2件" />
            </dd>
          </dl>
          ...
          <dl class="req">
            <dt>他者借入金額</dt>
            <dd>
              <input readonly class="validate[required] chk" type="text" id="your_borrowing_num2" name="your_borrowing_num2"
          ...
          <dl class="any">
            <dt>その他ご要望など</dt>
            <dd>
              <textarea readonly name="your_message" id="your_message"></textarea>
            </dd>
          </dl>

          <div class="checkbox-wrapper">
            <input type="checkbox" name="isconsult" value="同意する" id="agreement" checked>
            <label for="agreement">
              <p class="checkbox-txt">当サイトを金融業者ではなく、ご紹介サイトと理解して送信する事に同意する。</p>
            </label>
          </div>
          
          <div class="buttons-wrapper">
            <button onclick="alert('申込みが完了しました。');" type="submit" name="submit" class="btn_submit">送信</button><br>
            <button onclick="redirect_top()" type="reset" name="backpage"
              class="btn_back">フォームに戻る</button>
          </div>
        </form>
      </div>
    </section>
    <!-- <a id="submitMail" href="#" class="submit" value="送信">asdf</a> -->
    <!-- <a class="backpage" value="フォームに戻る"> asdfasdf</a> -->
  </main>
  <script>
        const formSubmitted = localStorage.getItem('formSubmitted')
        if (formSubmitted !== 'display' && formSubmitted !== null) {
