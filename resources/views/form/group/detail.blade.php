...
<form name="form1" id="form1" method="post" action="{{route('form.group.update')}}" class="form-horizontal">
	@csrf
		...
		<div class="col-md-8 col-lg-9">
						...
						<div class="box-body">
							<div class="form-group @if($errors->has('name')) has-error @endif">
								<div class="col-sm-12 col-xs-12">
									...
									@endif
									<input type="text" name="name" value="{{$group->name}}" class="form-control input-lg" id="name" placeholder="タイトルを入力してください">
								</div>
							</div>
							@if($mode == 'add') 
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="box box-solid">
						<!-- box-body Start -->
						<div class="box-body">
							<h3 class="page-header">メール設定（このグループに所属する運営者向けメール）</h3>
							<!-- row Start -->
							<div class="row">
								<div class="col-sm-12">
									<div class="form-group @if($errors->has('from_name')) has-error @endif">
										...
										@endif
											<input type="text" name="from_name" value="{{$group->from_name}}" class="form-control" id="from_name">
										</div>
									</div>
									<div class="form-group @if($errors->has('from_email')) has-error @endif">
										...
										<div class="col-sm-10 col-xs-12">
											...
											@endif
											<input type="text" name="from_email" value="{{$group->from_email}}" class="form-control" id="from_email">
										</div>
									</div>
									<div class="form-group @if($errors->has('mail_title')) has-error @endif">
										...
										<div class="col-sm-10 col-xs-12">
											...
											@endif
											<input type="text" name="mail_title" value="{{$group->mail_title}}" class="form-control" id="mail_title">
										</div>
									</div>
									<div class="form-group @if($errors->has('mail_body')) has-error @endif">
										...
										<div class="col-sm-10 col-xs-12">
											...
											@endif
											<textarea name="mail_body" rows="10" class="form-control" id="mail_body">{{$group->mail_body}}</textarea>
										</div>
									</div>
								</div>
							</div>
			...
			<div class="row">
				...
				<div class="col-sm-12">
					...
					<div class="box box-solid">
						<div class="box-body">
							<h3 class="page-header">除外設定</h3>
							<div class="form-group ">
								<label for="from_email" class="col-sm-2 control-label">生年月日</label>
								<div class="col-sm-10 col-xs-12">
									<input type="number" min="1940" max="2004" step="1" name="except_year" value="{{$group->except_year}}" class="form-control" id="except_year">
								</div>
							</div>
							@php
							...
							<div class="form-group ">
								...
								<div class="col-sm-10 col-xs-12">
									<select name="except_expectation[]" id="expectation" class="form-control select2" multiple>
										
										@foreach($expectation as $key => $val)
										<option value="{{$key}}" @if(in_array($key,$except_expectation)) selected @endif >{{$val}}</option>
										@endforeach
									</select>
								</div>
							</div>
							<div class="form-group ">
								<label for="from_email" class="col-sm-2 control-label">職業</label>
								<div class="col-sm-10 col-xs-12">
									<select name="except_job[]" id="job" class="form-control select2" multiple>
										@foreach($job as $key => $val)
										<option value="{{$key}}" @if(in_array($key,$except_job)) selected @endif >{{$val}}</option>
										@endforeach
									</select>
								</div>
							</div>
							<div class="form-group ">
								<label for="from_email" class="col-sm-2 control-label">都道府県</label>
								<div class="col-sm-10 col-xs-12">
									<select name="except_pref[]" id="pref" class="form-control select2" multiple>
										@foreach($pref as $key => $val)
										<option value="{{$key}}" @if(in_array($key,$except_pref)) selected @endif>{{$val}}</option>
										@endforeach
									</select>
								</div>
							</div>
									...
									<div class="form-group @if($errors->has('status')) has-error @endif">
										...
										<div class="col-sm-8 col-xs-12">
											...
											<select name="status" class="form-control" id="status">
												...
												<option label="{{$item}}" value="{{$key}}" @if($group->status == $key) selected="selected" @endif>{{$item}}</option>
												@endforeach
											</select>
										</div>
									</div>
